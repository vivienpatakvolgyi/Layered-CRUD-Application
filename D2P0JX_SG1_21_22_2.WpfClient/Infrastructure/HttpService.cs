...
namespace D2P0JX_SG1_21_22_2.WpfClient.Infrastructure
{
    public class HttpService
    {
        ...
        public ApiResult DeleteEntity<T>(T entity, string actionName = null)
        {
            using (var client = new HttpClient())
            {
                InitClient(client);

                var response = client.DeleteAsync(GetActionName(actionName ?? nameof(DeleteEntity))).GetAwaiter().GetResult(); // Block here
                return JsonSerializer.Deserialize<ApiResult>(response.Content.ReadAsStringAsync().GetAwaiter().GetResult(), serializerOptions);
            }
        }
